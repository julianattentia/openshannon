...
- **Per-agent model/executor routing** — Each agent declares a `modelTier` (`small`/`medium`/`large`) in `apps/worker/src/session-manager.ts`, which selects the Claude model via `resolveModel(tier)` (`apps/worker/src/ai/models.ts`). Per-agent *executor* routing is set via the optional `agent_executors` config map (`{ <agent-name>: claude | hermes }`), resolved by `resolveAgentExecutorId` (`apps/worker/src/ai/executor/select.ts`) with precedence: config map → the agent's built-in `executor` default → `SHANNON_EXECUTOR` → global default (`hermes`). `runPreflightChecks` validates Claude credentials whenever any agent routes to Claude (`usedExecutors` in `apps/worker/src/services/preflight.ts`). NOTE: the Hermes executor is single-model and ignores `modelTier` — tiering only affects agents routed to Claude.
- **Per-agent Hermes model routing** — Operators freely configure which Hermes model runs which agent. The config defines named targets in `hermes_models` (`{ <label>: { provider, model, base_url } }`) and assigns agents in `agent_models` (`{ <agent-name|default>: <label> }`). `resolveHermesTargetForAgent` (`apps/worker/src/ai/executor/hermes/model-target.ts`) resolves per agent with precedence: `agent_models[agent]` → `agent_models.default` → the single `SHANNON_HERMES_{PROVIDER,MODEL,BASE_URL}` env (so unconfigured runs are unchanged). The resolved target is threaded via `selectAgentExecutor({ hermes })` and synthesized into a per-run Hermes `config.yaml`. **Auth for providers like Nous (OAuth):** set `SHANNON_HERMES_SEED_DIR` (or rely on auto-detected `~/.hermes`); `resolveHermesSeed` (`apps/cli/src/paths.ts`) mounts it read-only at `/hermes-seed`, and the executor (`deriveRunHermesHome`) seeds `auth.json` + `shared/` from `HERMES_SEED_DIR` into a writable per-run `HERMES_HOME` (so the Nous token-refresh patch can rewrite its own copy). config.yaml is always synthesized from the resolved target — never seeded — so the local `custom` provider and a synthesized Nous provider both work (the OAuth patch keys off the provider name).
