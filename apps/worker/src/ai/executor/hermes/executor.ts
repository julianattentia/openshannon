...
export class HermesAgentExecutor implements AgentExecutor {
  ...
  private async deriveRunHermesHome(agentName: string): Promise<string> {
    ...
    // Credentials are SEEDED from `seedDir` (read-only is fine — e.g. a mounted
    // ~/.hermes), but per-run state lives under the writable `base`. This lets
    // the worker mount Nous OAuth read-only while the per-run copy (which the
    // OAuth-refresh patch may rewrite) stays writable. NOTE: config.yaml is
    // intentionally NOT seeded — we always synthesize it from the resolved
    // (provider, model, base_url) so per-agent model routing works and the
    // local `custom` provider stays defined. The OAuth patch keys off the
    // provider name, so a synthesized Nous provider authenticates fine.
    ...
    // An operator may still pin a full config.yaml under the writable HERMES_HOME
    // base (legacy path); that wins over synthesis. The seed dir provides only
    // credentials.
