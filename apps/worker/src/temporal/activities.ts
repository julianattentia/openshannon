...
// The MCP collectors are populated only when the agent calls their SDK tools,
// which requires the Claude Agent SDK channel. The Hermes executor has no such
// channel, so its collectors stay empty and the agent instead writes the
// deliverable directly via the `save-deliverable` tool. When nothing was
// collected, keep that agent-written file rather than overwriting it with an
// empty rendered placeholder.
function anyCollectorToolCalled(callStatus: object): boolean {
  return Object.values(callStatus).some((status) => status === 'called');
}
...
export async function runPreReconAgent(input: ActivityInput): Promise<AgentMetrics> {
  ...
  if (anyCollectorToolCalled(callStatus)) {
    ...
    logger.info('No pre-recon collector tools called; keeping agent-written deliverable (Hermes save-deliverable path)');
  }
...
export async function runReconAgent(input: ActivityInput): Promise<AgentMetrics> {
  ...
  if (anyCollectorToolCalled(callStatus)) {
    ...
    logger.info('No recon collector tools called; keeping agent-written deliverable (Hermes save-deliverable path)');
  }
...
async function runVulnAgentWithCollector(
  ...
  if (anyCollectorToolCalled(callStatus)) {
    ...
    logger.info(
      `No ${vulnClass} vuln collector tools called; keeping agent-written deliverable (Hermes save-deliverable path)`,
    );
  }
...
async function runExploitAgentWithCollector(
  ...
  if (collected.length > 0) {
    ...
    logger.info(
      `No ${vulnClass} exploit collector entries; keeping agent-written evidence (Hermes save-deliverable path)`,
    );
  }
